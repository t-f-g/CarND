    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          long int latency_ms = 100;
          const double latency_seconds = latency_ms / 1000.0;
          ...
          */
          ...
          // Transform points on vehicle position and orientation
          ...
          for(unsigned int i = 0; i < ptsx.size(); i++) {
            ptsx_transform[i] = (ptsx[i] - px) * cos(psi) + (ptsy[i] - py) * sin(psi);
          ...
          for(unsigned int i = 0; i < ptsy.size(); i++) {
            ptsy_transform[i] = (ptsy[i]- py) * cos(psi) - (ptsx[i] - px) * sin(psi);
          ...
          // Linear approximation of latency
          ...
          state << latency_px, 0, latency_psi, velocity, cte, epsi;
